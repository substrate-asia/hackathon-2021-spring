...
groups: ## 群组配置
  - name: group1 ## 群组名称
    peers: ## 群组下属节点
      - name: peer1 ## 节点名称
        is_root: true ## 是否为网络 Root 节点（拥有 Sudo 权限），默认值为：false
        is_group_main: true ## 是否为群组的主节点（用于管理群组内其他普通节点），默认值为：false
        ip: 127.0.0.1 ## 节点 IP
        port: 30333 ## 节点监听端口
        ws_port: 9944 ## 节点 websocket 监听端口
        rpc_port: 9933 ## 节点 rpc 监听端口
      - name: peer2
        ip: 127.0.0.1
        port: 30334
        ws_port: 9945
...
通过 [tools](../tools/README_CN.md) 中的工具完成必要的初始化以及合约部署。

最后恭喜您，您的网络已经成功启动并时刻准备着为世界提供服务。