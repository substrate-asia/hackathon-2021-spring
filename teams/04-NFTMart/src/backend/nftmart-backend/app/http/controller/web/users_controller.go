...
import (
	...
	"nftmart/app/model"
	"nftmart/app/service/users/curd"
...
// 2.1 用户钱包登录 只生成token 返回即可
func (u *Users) WalletLogin(context *gin.Context) {
	//  由于本项目骨架已经将表单验证器的字段(成员)绑定在上下文，因此可以按照 GetString()、GetInt64()、GetFloat64（）等快捷获取需要的数据类型，注意：相关键名规则：  前缀+验证器结构体中的 json 标签
	// 当然也可以通过gin框架的上下文原始方法获取，例如： context.PostForm("user_name") 获取，这样获取的数据格式为文本，需要自己继续转换
	pubKey := context.GetString(consts.ValidatorPrefix + "address")
	userTokenFactory := userstoken.CreateUserFactory()
	if userToken, err := userTokenFactory.GeneratePubKeyToken(pubKey, variable.ConfigYml.GetInt64("Token.JwtTokenCreatedExpireAt")); err == nil {
		// 记录登录TOKEN
		if userTokenFactory.RecordPKeyToken(userToken, context.ClientIP()) {
			data := gin.H{
				"token":      userToken,
...
// 根据钱包地址查询用户信息
func (u *Users) ShowUserByAddress(context *gin.Context) {
	pubKey := context.GetString("pubKey")
	user := model.CreateUserFactory("").ShowUserByAddress(pubKey)
	if user.Id != 0 {
...
func (u *Users) Update(context *gin.Context) {
	...
	nickName := context.GetString(consts.ValidatorPrefix + "nick_name")
	profilePhoto := context.GetString(consts.ValidatorPrefix + "profile_photo")
	pubKey := context.GetString("pubKey")
	email := context.GetString(consts.ValidatorPrefix + "email")
	twitter := context.GetString(consts.ValidatorPrefix + "twitter")
	userIp := context.ClientIP()
	//注意：这里没有实现权限控制逻辑，例如：超级管理管理员可以更新全部用户数据，普通用户只能修改自己的数据。目前只是验证了token有效、合法之后就可以进行后续操作
	// 实际使用请根据真是业务实现权限控制逻辑、再进行数据库操作
	if curd.CreateUserCurdFactory().SaveOrUpdate(id, nickName, profilePhoto, pubKey, email, twitter, userIp) {
		response.Success(context, consts.CurdStatusOkMsg, "")
	} else {
