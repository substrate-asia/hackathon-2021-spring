...
type WalletLogin struct {
	Address    string `form:"address" json:"address" binding:"required,min=10,max=300"` //  validate the pub_key
}

// 验证器语法，参见 Register.go文件，有详细说明

func (l WalletLogin) CheckParams(context *gin.Context) {

	//1.基本的验证规则没有通过
	if err := context.ShouldBind(&l); err != nil {
		errs := gin.H{
			"tips": "UserRegister参数校验失败，参数不符合规定，address不合法，不允许登录",
			"err":  err.Error(),
		}
	...
	if extraAddBindDataContext == nil {
		...
		(&web.Users{}).WalletLogin(extraAddBindDataContext)
	}
