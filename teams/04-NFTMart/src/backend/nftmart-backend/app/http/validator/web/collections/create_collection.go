...
type CreateCollection struct {
	//CategoryId float64 `form:"collectionId" json:"collectionId" binding:"required"`
	Name     string `form:"name" json:"name" binding:"required,min=2"`
	Icon string `form:"icon" json:"icon" binding:"required,min=8"`
	Describe string `form:"describe" json:"describe"`
}

// 验证器语法，参见 Register.go文件，有详细说明

func (s CreateCollection) CheckParams(context *gin.Context) {
	//1.基本的验证规则没有通过
	if err := context.ShouldBind(&s); err != nil {
		errs := gin.H{
			"tips": "CreateCollection参数校验失败，参数校验失败，name(>=2)、icon(>8)",
			"err":  err.Error(),
		}
	...
	if extraAddBindDataContext == nil {
		...
		(&web.Collections{}).CreateCollection(extraAddBindDataContext)
	}
}
