...
import (
	...
	"nftmart/app/http/controller/chaptcha"
	"nftmart/app/http/controller/graphql"
	"nftmart/app/http/middleware/authorization"
	"nftmart/app/http/middleware/cors"
	validatorFactory "nftmart/app/http/validator/core/factory"
	"io"
	"net/http"
	"os"
)
...
func InitWebRouter() *gin.Engine {
	...
	backend := router.Group("/admin/")
	{
		...
		noAuth := backend.Group("users/")
		{
			...
			noAuth.POST("walletlogin",validatorFactory.Create(consts.ValidatorPrefix+"UsersWalletLogin"))
		...
		backend.Use(authorization.CheckAuth())
		{
			...
			users := backend.Group("users/")
			{
				...
				// 地址查询
				users.POST("getUserByAddress",validatorFactory.Create(consts.ValidatorPrefix+"showUserByAddress"))
				// 新增
			...
			// collection router groups
			collections := backend.Group("collections/")
			{
				// create collection router
				collections.POST("createCollection", validatorFactory.Create(consts.ValidatorPrefix+"createCollection"))
				// show collect list
				collections.POST("showCollectionList", validatorFactory.Create(consts.ValidatorPrefix+"showCollectionList"))
			}

			// nft asset router groups
			assets := backend.Group("assets/")
			{
				// create nft asset router
				assets.POST("createAsset", validatorFactory.Create(consts.ValidatorPrefix+"createAsset"))
				// nft asset list router
				assets.POST("showAssets", validatorFactory.Create(consts.ValidatorPrefix+"showAsset"))
				// nft asset graphql search router
				assets.POST("/graphql", (&graphql.GraphController{}).GraphqlPOST())
			}
			//文件上传公共路由
			uploadFiles := backend.Group("upload/")
			{
				uploadFiles.POST("files", validatorFactory.Create(consts.ValidatorPrefix+"UploadFiles"))
				uploadFiles.POST("ipfsUpload", validatorFactory.Create(consts.ValidatorPrefix+"UploadFiles"))
			}
		}
	}
	return router
