...
export default function PolkadotContentMain(props){
  const classes = useStyles();
	...
	if (props) {
		console.log(state)
		const address = () => {
			return <Typography>{props.address}</Typography>
		}
		const network = () => {
			return <><TextField label="Network" defaultValue={props.defaultNetwork} value={state.defaultNetwork} onChange={evt => {
				let newState = state
				newState.defaultNetwork = evt.target.value
				setState(newState)
			}}
				variant="outlined" type="text" size="small" className={classes.textField} />
		...
		const items = [
			{ label: "Address", render: address },
			{ label: "Network", render: network },
			...
			<Grid container>
				...
				<Grid item xs={10} className={classes.box}>
					...
					<List>
						...
						<ListItem>
							<ListItemText primary={"Send DOT to: "} />
							<TextField className={classes.text_field}
								value={state.new_token_contract} onChange={(evt) => {
									setState({
										...state,
										new_token_contract: evt.target.value
									})
								}}
								label="Send To" />
					...
					<List>
						...
						<ListItem className={classes.listItem}>
							<ListItemText primary={"DOT Balance: "} />
							183.72 DOT
						</ListItem>
						<ListItem className={classes.listItem}>
							<ListItemText primary={"WETH Balance: "} />
							2.38 wETH
						</ListItem>
						<ListItem className={classes.listItem}>
							<ListItemText primary={"WBTC Balance: "} />
							0.0082 wBTC
						</ListItem>
						<ListItem>
							<ListItemText primary={"Add From Address: "} />
							<TextField className={classes.text_field}
								value={state.new_token_contract} onChange={(evt) => {
									setState({
										...state,
