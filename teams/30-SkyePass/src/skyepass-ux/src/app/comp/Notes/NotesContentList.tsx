...
import NotesContentMain from './NotesContentMain'
...
export default function NotesContentList() {
	...
	const polkadot = [{
		name: "Github Backup Code",
		excerpt: "...",
		content: "jkjk jkj klj kljkl jkl jkjkjlk jagsdhfsjkdfhh jshdfk jashdfjk sadfhjk shdfjhsdf"
	}, {
		name: "Some random journal",
		excerpt: "some random thoughts on 03/09/2021",
		content: "- kjalkdsjflkaj - kjlasdjfkljasdlkfjkl"
	}, {
		name: "Ramdom stuff",
		excerpt: "...",
		content: ""
				...
				<List classNames={classes.listContainer}>
					{polkadot.map((item, index) => {
						return <ListItem button className={classes.listItem} key={index}
							...
							<ListItemText primary={item.name} secondary={item.excerpt} />
						</ListItem>
					})}
				</List>
				<Drawer anchor="right" open={state.open} onClose={() => setState({ open: false })}>
					{NotesContentMain(state.item)}
				</Drawer>
			</Grid>
		</Grid>
	);
