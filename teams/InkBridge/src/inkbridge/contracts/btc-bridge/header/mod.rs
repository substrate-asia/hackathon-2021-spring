...
///           97             98    99   100     (height)
///
fn look_back_confirmed_header(
    store: &BtcBridge,
    header_info: &BtcHeaderInfo,
) -> (Option<BtcHeaderIndex>, Vec<BtcHeaderIndex>) {
    let confirmations = store.confirmation_number;
    ...
    for _cnt in 1..confirmations {
        if let Some(current_info) = store.headers.get(&prev_hash) {
            let current_info = current_info.1;
            chain.push(BtcHeaderIndex {
                hash: prev_hash,
...
pub fn update_confirmed_header(
    store: &mut BtcBridge,
    header_info: &BtcHeaderInfo,
) -> Option<BtcHeaderIndex> {
    let (confirmed, chain) = look_back_confirmed_header(store, header_info);
    for index in chain {
        set_main_chain(store, index.height, index.hash);
    }
    confirmed.map(|index| {
        store.confirmed_index = index;
        index
    })
}

fn set_main_chain(store: &mut BtcBridge, height: u32, main_hash: Hash) {
    if let Some(hashes) = store.block_hash_for.get(&height) {
        if hashes.len() == 1 {
            store.main_chain.insert(*&hashes[0], true);
            return;
        }
        for hash in hashes {
            if *hash == main_hash {
                store.main_chain.insert(*hash, true);
            } else {
                store.main_chain.take(hash);
            }
        }
    }
}

pub fn check_confirmed_header(store: &BtcBridge, header_info: &BtcHeaderInfo) -> Result<()> {
    let (confirmed, _) = look_back_confirmed_header(store, header_info);
    // if let Some(current_confirmed) = ConfirmedIndex::get() {
    let current_confirmed = store.confirmed_index;
