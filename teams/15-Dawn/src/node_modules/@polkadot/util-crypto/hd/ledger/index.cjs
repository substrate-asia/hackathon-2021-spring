...
function hdLedger(mnemonic, path) {
  ...
  return (0, _index2.naclKeypairFromSeed)(path.split('/').slice(1).map(n => parseInt(n.replace("'", ''), 10)).map(n => n < _validatePath.HARDENED ? n + _validatePath.HARDENED : n).reduce((x, n) => (0, _derivePrivate.ledgerDerivePrivate)(x, n), (0, _master.ledgerMaster)(mnemonic)).slice(0, 32));
}