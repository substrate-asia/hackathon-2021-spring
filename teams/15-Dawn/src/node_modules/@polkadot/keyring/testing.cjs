...
{
  name: 'Gerald',
  publicKey: new Uint8Array([3, 98, 79, 114, 14, 174, 103, 106, 4, 17, 22, 49, 201, 202, 51, 140, 17, 208, 245, 168, 14, 228, 34, 16, 198, 190, 114, 152, 60, 235, 98, 15, 191]),
  secretKey: (0, _util.hexToU8a)('0x99b3c12287537e38c90a9219d4cb074a89a16e9cdb20bf85728ebd97c343e342'),
  seed: "m/44'/40'/0'/0",
  type: 'ethereum'
}];
/**
...
function createTestKeyring(options = {}, isDerived = true) {
  const keyring = new _index2.default(options);
  ...
  pairs.forEach(({
    ...
    const pair = !isDerived && !name ? keyring.addFromUri(seed, meta, options.type) : keyring.addPair((0, _index.createPair)({
