...
const infoMapping = {
  ...
  [_types.TypeDefInfo.DoNotConstruct]: (registry, value) => _index2.DoNotConstruct.with(value.displayName),
  ...
  [_types.TypeDefInfo.Vec]: (registry, value) => {
    ...
    return subType === 'u8' ? createClass(registry, 'Bytes') : _index.Vec.with(subType);
  },
  [_types.TypeDefInfo.VecFixed]: (registry, {
    ...
    return sub.type === 'u8' ? _index.U8aFixed.with(length * 8, displayName) : _index.VecFixed.with(sub.type, length);
  }
