...
class RpcCore {
  ...
  _createMethodSend(section, method, def) {
    ...
    const callWithRegistry = async (outputAs, values) => {
      ...
      const {
        registry
      } = blockHash && (0, _classPrivateFieldLooseBase2.default)(this, _getBlockRegistry)[_getBlockRegistry] ? await (0, _classPrivateFieldLooseBase2.default)(this, _getBlockRegistry)[_getBlockRegistry](blockHash) : {
  ...
  _newType(registry, blockHash, key, input, isEmpty, entryIndex = -1) {
    ...
    if (meta.modifier.isOptional) {
      ...
      const option = new _types.Option(registry, (0, _create.createClass)(registry, type), inner);
    ...
    try {
      ...
      l.error(`Unable to decode storage ${key.section || 'unknown'}.${key.method || 'unknown'}:${entryNum}`, error.message);
      return (0, _create.createTypeUnsafe)(registry, 'Raw', [input], {
        blockHash
      });
    }
  }
