...
function signingHeader(api) {
  return (0, _xRxjs.combineLatest)([api.rpc.chain.getHeader(), api.rpc.chain.getFinalizedHead()]).pipe((0, _operators.switchMap)(([bestHeader, finHash]) => // retrieve the headers - in the case of the current block, we use the parent
  ...
  bestHeader.parentHash.isEmpty ? (0, _xRxjs.of)([bestHeader, bestHeader]) : (0, _xRxjs.combineLatest)([api.rpc.chain.getHeader(bestHeader.parentHash), api.rpc.chain.getHeader(finHash)])), (0, _operators.map)(([current, finalized]) => // determine the hash to use, current when lag > max, else finalized
